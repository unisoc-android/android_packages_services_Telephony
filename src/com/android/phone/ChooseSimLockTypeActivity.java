...
public class ChooseSimLockTypeActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.one_key_simlock);

        mTypeListView = (ListView)findViewById(R.id.listview);
        mSimLockType = getResources().getStringArray(R.array.one_key_sim_lock_type);
        mTypeList = Arrays.asList(mSimLockType);
        ArrayAdapter<String> arrayAdapter = new ArrayAdapter<String>
                            (this,android.R.layout.simple_list_item_1, mTypeList);
        mTypeListView.setAdapter(arrayAdapter);
        mTypeListView.setOnItemClickListener(new OnItemClickListenerImpl());
    ...
    private void setFacilityLockByUser(int type, boolean enable) {
        ...
        Log.d(TAG, "setFacilityLockByUser type = " + type + ", enable = " + enable);
        mRadioInteractor.setFacilityLockByUser(
                mFacility, enable, mHandler.obtainMessage(MyHandler.MESSAGE_SET_FACILITY_LOCK_DONE), 0);
    }
    ...
    private void querySimLockState(int type) {
        ...
        ((GsmCdmaPhone)mPhone).mCi.queryFacilityLock(mFacility,"00000000", DEFAULT_SERVICE_CLASS,
                mHandler.obtainMessage(MyHandler.MESSAGE_QUERY_FACILITY_LOCK_DONE));
    }
    ...
    private void showSimLockDialog(final int type,final boolean enable) {
        ...
        builder.setPositiveButton(lockedButtonID, new DialogInterface.OnClickListener() {
                public void onClick(DialogInterface dialog, int which) {
                    mTypeListView.setEnabled(false);
                    setFacilityLockByUser(type, enable);
                }
        });
        builder.setNegativeButton(R.string.cancel, new DialogInterface.OnClickListener() {
                public void onClick(DialogInterface dialog, int which) {
                    mTypeListView.setEnabled(true);
                }
        });
    ...
    private class MyHandler extends Handler {
        private static final int MESSAGE_QUERY_FACILITY_LOCK_DONE = 0;
        private static final int MESSAGE_SET_FACILITY_LOCK_DONE = 1;
        private static final int EVENT_REBOOT = 500;

        public void handleMessage(Message msg) {
            AsyncResult ar;
            switch (msg.what) {
                case MESSAGE_QUERY_FACILITY_LOCK_DONE:
                    ar = (AsyncResult) msg.obj;
                    if (ar.exception != null) {
                        mTypeListView.setEnabled(true);
                        Toast.makeText(mPhone.getContext(), R.string.one_key_query_unsuccess, Toast.LENGTH_LONG).show();
                        Log.d(TAG, "MESSAGE_QUERY_FACILITY_LOCK_DONE: ar.exception=" + ar.exception);
                    } else {
                        int infoArray[] = (int[]) ar.result;
                        showSimLockDialog(mCurrentSimLockType, (infoArray[0] != 1));
                    }
                    break;
                case MESSAGE_SET_FACILITY_LOCK_DONE:
                    ar = (AsyncResult) msg.obj;
                    if (ar.exception != null) {
                        mTypeListView.setEnabled(true);
                        Log.d(TAG, "MESSAGE_SET_FACILITY_LOCK_DONE: ar.exception=" + ar.exception);
